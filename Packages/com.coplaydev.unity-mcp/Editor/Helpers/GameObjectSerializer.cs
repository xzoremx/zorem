    ...
    public static class GameObjectSerializer
    {
        ...
        public static object GetGameObjectData(GameObject go)
        {
            ...
            return new
            {
                ...
                parentInstanceID = go.transform.parent?.gameObject.GetHashCode() ?? 0, // 0 if no parent
        ...
        {
            // --- Add Early Logging --- 
            // McpLog.Info($"[GetComponentData] Starting for component: {c?.GetType()?.FullName ?? "null"} (ID: {c?.GetHashCode() ?? 0})");
            // --- End Early Logging ---
            ...
            if (componentType == typeof(Transform))
            {
                ...
                // McpLog.Info($"[GetComponentData] Manually serializing Transform (ID: {tr.GetHashCode()})");
                return new Dictionary<string, object>
                {
                    { "typeName", componentType.FullName },
                    { "instanceID", tr.GetHashCode() },
                    ...
                    { "parentInstanceID", tr.parent?.gameObject.GetHashCode() ?? 0 },
                    { "rootInstanceID", tr.root?.gameObject.GetHashCode() ?? 0 },
                    { "childCount", tr.childCount },
                    // Include standard Object/Component properties
                    { "name", tr.name },
                    { "tag", tr.tag },
                    { "gameObjectInstanceID", tr.gameObject?.GetHashCode() ?? 0 }
                };
            }
            ...
            if (componentType == typeof(Camera))
            {
                ...
                var safeProperties = new Dictionary<string, Func<object>>
                {
                    ...
                    { "name", () => cam.name },
                    { "tag", () => cam.tag },
                    { "gameObject", () => new { name = cam.gameObject.name, instanceID = cam.gameObject.GetHashCode() } }
                };
